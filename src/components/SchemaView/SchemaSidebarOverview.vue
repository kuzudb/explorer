  ...
  <div>
    <div>
      <div class="d-flex justify-content-between">
        <h5>Node Tables</h5>
        <button v-if="modeStore.isReadWrite" class="btn btn-sm btn-[var(--bs-body-accent)]"
      ...
      <table v-if="schema" class="table table-sm table-borderless schema_side-panel__overview-table">
        <tbody>
          <tr v-for="nodeTable in schema.nodeTables" :key="nodeTable.name">
            <td scope="row">
              <span class="badge bg-[var(--bs-body-accent)]"
            ...
            <td v-if="modeStore.isReadWrite" class="schema_side-panel__overview-table-buttons-container">
              <div>
                <button class="btn btn-sm btn-outline-[var(--bs-body-accent)]" @click="editTable(nodeTable.name)">
                  <i class="fa-solid fa-pencil" />
                </button>
                &nbsp;
                <button class="btn btn-sm btn-outline-[var(--bs-body-accent)]" @click="dropTable(nodeTable.name)">
      ...
      <table v-if="schema" class="table table-sm table-borderless schema_side-panel__overview-table">
        <tbody>
          <tr v-for="relTable in schema.relTables" :key="relTable.name">
            <td scope="row" :colspan="relTable.group ? 2 : 1">
            ...
            <td v-if="modeStore.isReadWrite && !relTable.group"
              class="schema_side-panel__overview-table-buttons-container">
              <div>
                <button class="btn btn-sm btn-outline-[var(--bs-body-accent)]" @click="editTable(relTable.name)">
                  <i class="fa-solid fa-pencil" />
                </button>
                &nbsp;
                <button class="btn btn-sm btn-outline-[var(--bs-body-accent)]" @click="dropTable(relTable.name)">
                  <i class="fa-solid fa-trash-can" />
                </button>
              </div>
            </td>
